...
if not df_movies.empty:
    print("👥 Gerando usuários e avaliações...")
    df_users, df_ratings = generate_mock_data(df_movies)
    
    print(f"👤 Usuários gerados: {len(df_users)}")
    print(f"⭐ Avaliações geradas: {len(df_ratings)}")
    
    # Criar pasta e salvar
    os.makedirs('../data-lake', exist_ok=True)
    
    df_movies.to_csv('../data-lake/movies.csv', index=False)
    df_users.to_csv('../data-lake/users.csv', index=False)
    df_ratings.to_csv('../data-lake/ratings.csv', index=False)
    
    print(f"\n✅ Arquivos salvos em ../data-lake/:")
    print(f"   - movies.csv ({len(df_movies)} registros)")
    print(f"   - users.csv ({len(df_users)} registros)")
    print(f"   - ratings.csv ({len(df_ratings)} registros)")
else:
    print("⚠️ Nenhum filme encontrado. Verifique sua API Key ou os termos de busca.")